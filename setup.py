...
if WINDOWS_HOST:
    ...
    if DEFAULT_QMAKE == '':
        print('Unable to find qmake.exe.'
              'Please set the path manually using --qmake option.')
else:
    DEFAULT_MAKE = 'make'
...
class MyBuilderExt(build_ext):
    ...
    def finalize_options(self):
        ...
        if self.qt_include_dir is None:
            pipe = subprocess.Popen([self.qmake, '-query', 'QT_INSTALL_HEADERS'],
    ...
    def __build_qcustomplot_library(self):
        if WINDOWS_HOST:
            qcustomplot_static = join(BUILD_DIR, self.build_temp,
                                      'release', 'qcustomplot.lib')
        else:
        ...
        self.spawn([self.qmake, join(ROOT, 'src/qcp-staticlib.pro')])
...
setup(
    ...
    ext_modules=[
        Extension(
            'QCustomPlot_PyQt5',
            [join(ROOT, 'sip/all_PyQt5.sip')],
            include_dirs=[join(ROOT, 'src'), join(ROOT, 'sip')]
        ),
    ],
